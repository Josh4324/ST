...
describe("ST", function () {
  ...
  async function deploy() {
    ...
    const ST = await ethers.getContractFactory("ST2");
  ...
  describe("ST", function () {
    it("Test 1", async function () {
      const { st, user1, owner } = await loadFixture(deploy);
      const t1 = 1;
   ...
   */
      ...
      await st.connect(user1).payOrderInterChain();
      await st.editOrder(0, ethers.parseEther("5"), t4, 20);

      //console.log(await st.getOrder(0));
      await sleep(70000);

      await st.payOrderInterChain();
      await st.pauseOrder([0]);
      await st.startOrder([0]);
      await st.deleteOrder(0);

      console.log(await st.getOrders(owner.address));
      console.log(await st.getTransactions(owner.address));
